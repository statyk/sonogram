...
- Signing string (exact, both languages): `{agent}\n{timestamp}\n{METHOD}\n{path_with_query}\n{sha256_hex_of_body}` where `agent` is the `x-sonogram-agent` header value, METHOD is uppercase, `path_with_query` is the URL path plus `?query` if present, and the body hash is lowercase hex of the raw request body bytes (empty body = hash of zero bytes). Binding the agent name into the signature ties it to the claimed identity and prevents cross-agent signature replay if a key is ever shared or reused.
